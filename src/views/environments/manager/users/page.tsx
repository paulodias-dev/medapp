...
export function UsersList() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['manager', 'users', page, search],
    queryFn: () => managerService.getUsers(page, search),
  });
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex flex-col gap-1">
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Usuários (Staff)</h1>
          <p className="text-slate-500 font-medium">Gerencie a equipe administrativa e médica.</p>
        </div>
        ...
        <CardContent className="p-0">
                ...
                {isLoading ? (
                  ...
                  <tr key={user.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4 hidden md:table-cell">
                      <span className="text-sm text-slate-600 font-medium">{user.phone || 'Sem telefone'}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold ${
                        user.status === 1 ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-500'
                      }`}>
          ...
          {!isLoading && data && data.total === 0 && (
            <div className="py-20 flex flex-col items-center justify-center text-slate-400">
              <UserList size={48} weight="duotone" className="mb-4 opacity-20" />
          ...
          {!isLoading && data && data.total > 0 && (
            <div className="px-6 py-4 border-t border-slate-50 flex items-center justify-between">
              <p className="text-xs text-slate-500 font-medium">
                Mostrando <span className="font-bold text-slate-900">{data.data.length}</span> de <span className="font-bold text-slate-900">{data.total}</span> usuários
              </p>
